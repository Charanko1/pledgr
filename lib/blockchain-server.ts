...
export const BOT_RPC_URL =
  ...
  process.env.NEXT_PUBLIC_BOT_RPC_URL ||
  DEFAULT_RPC_URL;
export const SERVER_CONTRACT_ADDRESS =
  process.env.TRUSTKAS_CONTRACT_ADDRESS ||
...
export function getServerProvider() {
  ...
  provider = new JsonRpcProvider(
    BOT_RPC_URL,
    {
      name: `bot-chain-${BOT_CHAIN_ID}`,
      chainId: BOT_CHAIN_ID,
    },
    { staticNetwork: true }
  );

  (provider as JsonRpcProvider & { pollingInterval?: number }).pollingInterval = BLOCKCHAIN_POLLING_INTERVAL_MS;

  return provider;
}

export function assertContractAddress() {
  if (!SERVER_CONTRACT_ADDRESS || !isAddress(SERVER_CONTRACT_ADDRESS)) {
    throw new Error("PLEDGR contract address is not configured correctly.");
  }
}
...
export async function getConfirmedTransaction(txHash: string) {
  ...
  if (!tx.to || tx.to.toLowerCase() !== SERVER_CONTRACT_ADDRESS.toLowerCase()) {
    throw new Error("Transaction was not sent to the PLEDGR contract.");
  }
...
export async function verifyContractEvent(
  ...
  const parsedTransaction = iface.parseTransaction({
    data: tx.data,
    value: tx.value,
  });
  const expectedFunction = FUNCTION_BY_EVENT[eventType];

  if (!parsedTransaction || parsedTransaction.name !== expectedFunction) {
    throw new Error(`Transaction does not call ${expectedFunction}().`);
  }

  const txProposalId = String(parsedTransaction.args[0]);
  if (txProposalId !== expectedProposalId) {
    throw new Error("Blockchain transaction does not match this proposal.");
  }
  ...
  const parsedEvent = receipt.logs
    .filter(
      (log) =>
        ...
        log.address.toLowerCase() === SERVER_CONTRACT_ADDRESS.toLowerCase()
    )
    .map((log) => {
      try {
  ...
  if (!parsedEvent) {
    throw new Error(`Transaction does not contain a valid ${eventType} event.`);
  }

  if (eventType === "Donated" && tx.value !== parsedEvent.args[1]) {
    throw new Error("Donation transaction value does not match the blockchain event.");
  }

  return { tx, receipt, parsedEvent };
}

export function resetServerBlockchainProvider() {
  provider?.destroy();
  provider = null;
}

import { Contract, formatEther, getAddress, isAddress, isHexString, verifyTypedData, TypedDataEncoder } from "ethers";
...
export async function prepareRegistration(p: any) {
  if (p.validationStatus !== "Approved" || p.adminReviewStatus !== "Approved") throw new Error("Validator and admin proposal reviews must be completed first.");
  ...
  const [validator, admin] = await Promise.all([User.findById(p.validatedBy), User.findById(p.adminReviewedBy)]);
  if (!validator?.walletAddress || !admin?.walletAddress) throw new Error("Both reviewers must have verified wallets.");
  await reviewer(p, validator, validator.walletAddress, "validator");
  await reviewer(p, admin, admin.walletAddress, "admin");
  if (getAddress(validator.walletAddress) === getAddress(admin.walletAddress)) throw new Error("Validator and admin must use distinct wallets.");
  const registration = { validator: validator.walletAddress, reviewerAdmin: admin.walletAddress, validUntil: Math.floor(Date.now()/1000)+86400, validatorSignature: "", adminSignature: "" };
  // Compare-and-set prevents a late prepare request discarding a signature.
  await Proposal.updateOne({ _id:p._id, blockchainStatus:"PENDING", $or:[{"registration.validUntil":{$exists:false}},{"registration.validUntil":{$lte:Math.floor(Date.now()/1000)}}] }, { $set:{registration} });
...
export async function signRegistration(p: any, user: any, role: "validator" | "admin", signature: string) {
  ...
  const result = await Proposal.updateOne({_id:p._id,blockchainStatus:"PENDING","registration.validUntil":message.validUntil},{$set:{[`registration.${role === "validator" ? "validatorSignature" : "adminSignature"}`]:signature}});
...
export async function reviewWithdrawal(p: any, user: any, requestId: string, role: "validator" | "admin", signature: string, reject: boolean) {
  ...
  const expected = role === "validator" ? "Requested" : "ValidatorApproved";
  if (w.status !== expected) throw new Error("This request is not awaiting your review.");
  if (!reject) {
  ...
  const update = reject ? {status:"Rejected",validatorSignature:"",adminSignature:""} : role === "validator" ? {status:"ValidatorApproved",validatorSignature:signature} : {status:"Approved",adminSignature:signature};
  const result = await WithdrawalRequest.updateOne({_id:w._id,requestId,status:expected},{$set:update});
...
export async function syncV2(p: any, txHash: string, group: any) {
  ...
  if (registration && (getAddress(tx.from)!==getAddress(p.recipientWallet) || p.validationStatus!=="Approved" || p.adminReviewStatus!=="Approved")) throw new Error("Registration was not submitted by the approved creator.");

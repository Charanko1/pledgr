            ...
            return (
              <div key={proposal._id} className="bg-white border p-5 shadow-brutal">
                <div className="flex flex-wrap gap-3 justify-between items-start">
                  ...
                  <span className={`text-xs px-3 py-1 rounded-full ${statusClass(proposal)}`}>{proposal.v2Withdrawal ? (proposal.v2Withdrawal.validUntil * 1000 <= Date.now() ? "Withdrawal expired" : proposal.v2Withdrawal.status === "Requested" ? "Awaiting required signatures" : proposal.v2Withdrawal.status === "AdminApproved" ? "Awaiting validator signature" : proposal.v2Withdrawal.status === "ValidatorApproved" ? "Awaiting admin signature" : "Ready for creator claim") : proposal.status}</span>
                </div>
                ...
                <div className="mt-5 flex gap-2 flex-wrap">
                  ...
                  {(proposal.contractVersion === 2 ? !isCreator && proposal.approvalPolicy?.requireValidator && proposal.validationStatus === "Pending" && ["Pending","Validated"].includes(proposal.status) : proposal.status === "Pending") && isValidator && <><button onClick={() => onValidate(proposal._id, "approve")} className="bg-green-600 text-white px-4 py-2 pledgr-action">Validate</button><button onClick={() => onValidate(proposal._id, "reject")} className="bg-red-600 text-white px-4 py-2 pledgr-action">Reject</button></>}
                  {(proposal.contractVersion === 2 ? !isCreator && proposal.approvalPolicy?.requireAdmin && proposal.adminReviewStatus === "Pending" && ["Pending","Validated"].includes(proposal.status) : proposal.status === "Validated") && isAdmin && <><button onClick={() => onAdminReview(proposal._id, "approve")} className="bg-green-600 text-white px-4 py-2 pledgr-action">Approve Proposal</button><button onClick={() => onAdminReview(proposal._id, "reject")} className="bg-red-600 text-white px-4 py-2 pledgr-action">Reject</button></>}
                  ...
                  {isFunding && finished && isCreator && <button onClick={() => onRequestWithdrawal(proposal._id)} className="bg-primary text-white px-4 py-2 pledgr-action">{proposal.status === "Release Rejected" ? "Request Again" : "Request Withdrawal"}</button>}
                  {proposal.status === "Withdrawal Requested" && isValidator && <><button onClick={() => onWithdrawalReview(proposal._id, "approve")} className="bg-green-600 text-white px-4 py-2 pledgr-action">Approve Release</button><button onClick={() => onWithdrawalReview(proposal._id, "reject")} className="bg-red-600 text-white px-4 py-2 pledgr-action">Reject Release</button></>}
                  {proposal.status === "Validator Release Approved" && isAdmin && <><button onClick={() => onWithdrawalReview(proposal._id, "approve")} className="bg-green-600 text-white px-4 py-2 pledgr-action">Final Approve Release</button><button onClick={() => onWithdrawalReview(proposal._id, "reject")} className="bg-red-600 text-white px-4 py-2 pledgr-action">Reject Release</button></>}
